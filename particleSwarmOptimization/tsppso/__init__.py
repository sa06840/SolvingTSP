...
def berlin52():
    ...
    dataset = TSPDataset()
    dataset.read('./particleSwarmOptimization/datasets/berlin52.tsp')
...
def zi929():
    from datasets import TSPDataset
    dataset = TSPDataset()
    dataset.read('./particleSwarmOptimization/datasets/zi929.tsp')
    dataset.unique(eps=1e-4, inplace=True)
    print('dataset size:', len(dataset.positions))
...
def lu634():
    from datasets import TSPDataset
    dataset = TSPDataset()
    dataset.read('./particleSwarmOptimization/datasets/lu634.tsp')
    dataset.unique(eps=1e-4, inplace=True)
    print('dataset size:', len(dataset.positions))
...
def canada():
    from datasets import TSPDataset
    dataset = TSPDataset()
    dataset.read('./particleSwarmOptimization/datasets/canada.tsp')
...
def qa194():
    from datasets import TSPDataset
    dataset = TSPDataset()
    dataset.read('./particleSwarmOptimization/datasets/qa194.tsp')
    dataset.unique(eps=1e-4, inplace=True)
    print('dataset size:', len(dataset.positions))
    ...
    problem = Problem(xy=dataset.positions)
    optimizer = TSPPSO(200, 100, p1=0.95, p2=0.03, p3=0.02, max_no_improv=3, rng=rng)
    solution = optimizer.minimize(problem)
